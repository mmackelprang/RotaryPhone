...
/// DEFERRED-ANSWER inbound path at the TRANSPORT level: an inbound INVITE is answered with 180 Ringing
/// only — the 200 OK is HELD until the handset is lifted
...
/// still FINDS the session, reads Cancelled, and correctly declines to send a 200 OK. The
/// REAL-path integration coverage lives in <c>GVApiAdapterInboundAnswerIntegrationTests</c>.
/// </summary>
public class GvSipTransportInboundDeferredAnswerTests
{
    ...
    // 5b) A CANCEL that arrives during ring must make a following AcceptIncomingCallAsync a no-op —
    //     NO held 200 OK is sent for a cancelled call. The session stays in _activeCalls as Cancelled
    //     (PR #40 regression guard), so the accept still FINDS it, reads Cancelled, and declines.
    [Fact]
    public async Task AcceptAfterCancel_DoesNotSendHeld200Ok()
    {
        ...
        // CANCEL arrives first and cancels the ringing call (caller hung up).
        fake.FeedMessage(InboundCancel(TestCallId));
        await WaitForAsync(() => fake.Sends.Any(s =>
            s.StartsWith("SIP/2.0 487 Request Terminated", StringComparison.Ordinal)));

        // Now the handset lifts a moment too late — accept must find the Cancelled session and bail
        // WITHOUT sending a 200 OK to a caller who is gone.
        await transport.AcceptIncomingCallAsync(TestCallId);
    ...
    // 7) PR #40 ROOT-CAUSE GUARD (transport level): a CANCEL during the ring must NOT evict the
    //    session — it stays in _activeCalls as Cancelled, so a following accept still finds it. This
    //    is the exact mechanism whose absence broke #40 (the CANCEL handler TryRemove'd the Ringing
    //    session, so AcceptIncomingCallAsync found nothing and never sent the held 200 OK).
    [Fact]
    public async Task CancelDuringRing_KeepsSessionFindable_ForLateAccept()
    {
        var fake = new FakeSipWebSocketChannel { OnSend = RegisterAutoResponder() };
        await using var transport = await RegisteredTransportWithRingingCallAsync(fake);
        ...
        // The session must still be present (status reads back, not Unknown) — i.e. it was NOT
        // evicted from _activeCalls. GetStatusAsync returns Unknown only when there is no session.
        var status = await transport.GetStatusAsync(TestCallId);
        Assert.Equal(CallStatusType.Cancelled, status.Status);
    }
}
