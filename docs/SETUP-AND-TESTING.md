...
> **⚠ `psidtsAgeSeconds` was REMOVED on 2026-09-08.** If you are looking for it, or find it in an older
> document: it measured the age of a *cache operation*, not of the credential — a mere cookie load
> restamped it, so it reset to ~0 on every restart and read reassuringly low for a credential that was
> days old. It was deleted rather than corrected, because a field named for credential age that reports
> something else is a trap regardless of what the docs say about it. Use `psidtsMintedAtUtc`, which
> measures the *credential* and is the only one of the two that could ever tell you the phone is about
> to stop working. See `docs/KNOWN-ISSUES.md` finding **L2**.
