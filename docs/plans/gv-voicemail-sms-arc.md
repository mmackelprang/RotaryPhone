...
- **Deferred to Planner — PR1 review HIGH-2:** `IGvAuthenticatedClientProvider.GetAuthenticatedClient()`
  ...
  → **SCHEDULED into B2, Task 1** (`docs/plans/gv-auth-blackout-b2-plan.md`). No longer a loose thread.
...
6. Reconcile the PR1 HIGH-2 auth-recovery window (Planner follow-up) — schedule it?
   → **Scheduled:** folded into the **B2** plan, Task 1 (`docs/plans/gv-auth-blackout-b2-plan.md`). A
