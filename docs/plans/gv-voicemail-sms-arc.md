...
| FF. GV mark-read (durable read-state) — Path A | 🚀 **in-flight (build approved by owner — branch `feat/gv-markread-path-a`)** | **Plan (path a): `docs/superpowers/plans/2026-06-20-gv-markread-readstate.md`.** Decision record: `docs/architecture/decisions/2026-06-20-gv-markread-readstate-contract.md`; reply to RadioConsole: `docs/handoffs/radioconsole-gv-markread-reply.md`; request: `docs/prompts/radioconsole-gv-markread-readstate-request.md`. **Owner-hold lifted; ships dark (`EnableMarkRead`=false). Path B (Task 9 poller-flip) NOT in this PR — still a fast-follow.** |
