...
| 2026-09-25 | RotaryPhone session (Builder) | **GV bridge Chrome command line: one added flag. No BT or audio change. `hci0`/`hci1` ownership, profiles and WirePlumber configs are untouched. Window stacking and focus are unchanged, and the kiosk process, profile and CDP port 9223 are not touched.** `gv-bridge-ensure.sh` now launches the bridge Chrome with **`--disable-backgrounding-occluded-windows`**, the same flag your `radio-kiosk-launch` already passes. **Intent:** the bridge window keeps rendering while your fullscreen kiosk covers it. The bridge stays *behind* the kiosk; it is never raised and never takes focus. **Why:** on 2026-09-25 the first unattended GV auto-relogin, run with the kiosk up, could not see the Google password field render in the covered window. ⚠ **This is hardening, not the fix.** A second real run on 2026-09-26 at 03:23Z put the kiosk deliberately in front of the bridge. The driver logged `document.visibilityState = 'visible'` for the whole 16 s, on `/v3/signin/challenge/pwd`, with the `Passwd` input present but 0×0. The page was **not** hidden or backgrounded, so the failure is not the one this flag addresses. ⚠ **Also, under `--ozone-platform=wayland` on GNOME 46 this flag is probably a no-op.** Chromium's Wayland backend never reports the `OCCLUDED` state the flag rewrites, and mutter withholds frame callbacks from a fully covered surface, which no Chrome flag overrides. Sources and the discriminating check are in RotaryPhone `docs/SETUP-GVBridge.md`, "The bridge window is covered by the kiosk". **Effect on the kiosk: expected none, not measured.** The flag changes only the bridge's own page-visibility bookkeeping, and the compositor still does not composite a covered window. If it did make the bridge render while covered, the cost would be the bridge's own CPU and GPU time on a box it shares with you. Baseline measured 2026-09-25, covered and without the flag: the bridge's 14 processes averaged **1.14 % of one core** at **858 MB RSS**. For comparison, your kiosk used about 16 CPU-min over the 4.8 h from 18:16 to 23:04 that day. The runbook re-measures after install. ⚠ **Installing it is entangled with the exit-code question.** The flag reaches the box only when `~/bin/gv-bridge-ensure.sh` is replaced. The installed copy is the 2026-08-18 13-line script, and replacing it also brings in the `flock`, whose lock-held path exits 0 without launching. That is the third exit-0 outcome from the 2026-09-08 row and from our spec §8. ADR `docs/architecture/decisions/2026-09-08-gv-bridge-ensure-exit-code.md` keeps exit 0 by contract, and your installed `radio-console-open` re-probes with `pgrep` after exit 0 (`:414`, `:473`), so we believe the new outcome is harmless to you. **We have not confirmed that with you, and will not install before you answer.** The question is drafted at `docs/handoffs/2026-09-25-radioconsole-ensure-install-exit-code-question-DRAFT.md` and is **not yet delivered**. ⚠️ **NOT MERGED (PR open for owner review), NOT DEPLOYED, NOT INSTALLED.** The running bridge (PID started 2026-09-20 03:01) keeps its old command line until an attended restart. |
