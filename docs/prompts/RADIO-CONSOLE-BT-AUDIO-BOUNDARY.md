...
> **Last updated:** 2026-09-08 by RotaryPhone session (Builder — **GV auth lineage fixed**: the four defects behind the 83-minute guest-facing outage; **no BT or audio behavior change**, and **no existing field renamed, moved, retyped or removed**). ⚠️ **Three items for Radio Console in the newest Change Log row:** **(1)** `/api/gvbridge/status` gains **`psidtsMintedAtUtc`** (nullable — **`null` means UNKNOWN, which is NOT healthy**, and it has **no upper bound**) plus `browserSessionValidatedAt` / `browserSessionAgeSeconds` / `browserSessionStale`; **(2)** **`psidtsAgeSeconds` is UNCHANGED but now DEPRECATED** — it reports the age of the last cookie *load*, not of the credential, which is how a two-day session death looked healthy; retiring it is an open owner call; **(3)** **`POST /api/gvbridge/cookies/refresh-from-browser` now answers `502` where it answered `200`** on a stale browser session, and no longer overwrites working credentials. ⚠️ **MERGED, NOT DEPLOYED.** Earlier: bell/phone-status contract — `ht801IpAddress` newly nullable over REST, and the `bell-failure/ack` body contradicts the delivered reply (`{"acknowledged": false}` on a repeat ack), pre-existing and deliberately unfixed, needs an owner call. Also still open: **`gv-bridge-ensure.sh` always exits 0, even when the launch fails**. Still open from earlier entries: the `authBlackout` sub-second caveat under Integration Points, and the deploy hazard that can rewrite RotaryPhone's `BluetoothAdapter` setting under Operational Notes.
>
...
Added by the fix for the 2026-09-08 outage. **Append-only; nothing existing changed name, position or type.**
...
⚠️ **`psidtsAgeSeconds` is DEPRECATED and its value is unchanged.** It measures the age of the last cookie
**load**, not of the credential: any load restamps it, so it resets to ~0 on every restart and reload, and
it read `208` for a credential minted two days earlier. **That is what let a two-day session death look
healthy.** Its behaviour is deliberately frozen for contract compatibility and pinned by tests; Radio
Console has retracted its published bands and confirmed zero code references, so **retiring it is an open
owner decision**. Use `psidtsMintedAtUtc` for anything new.
