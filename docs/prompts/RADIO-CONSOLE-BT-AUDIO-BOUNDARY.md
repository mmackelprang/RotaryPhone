...
| `browserRefreshOutcome` | ⭐ String, added 2026-09-09. The **whole** enum behind `browserSessionStale`: `NotAttempted`, `Unreachable`, `Stale`, `Succeeded`, `TornDown`, and since 2026-09-25 **`SignedOut`** (Chrome answered but holds no Google session; a human must sign in — `browserSessionStale` reads **false** for it). |
...
| 2026-09-25 | RotaryPhone session | **API only — one ADDITIVE enum value. No BT/audio change; hci0/hci1 ownership, profiles and WirePlumber configs untouched.** `GET /api/gvbridge/status`'s **`browserRefreshOutcome`** can now read **`SignedOut`**: Chrome answered on CDP but holds no Google session (its page is on the Google sign-in or the Workspace Voice landing page). Until now that state was reported as **`Unreachable`**, which is wrong: measured on the box 2026-09-25, a Chrome parked on `accounts.google.com/v3/signin/challenge/pwd` was reported `Unreachable`, and both the service log and the alarm told the owner to check whether Chrome was running when the fix was to sign in. Nothing else moves: no field is added, removed, renamed or reordered, and **`browserSessionStale` keeps its exact meaning** ("Chrome handed us cookies and Google rejected them"), so it reads **false** for `SignedOut`. **Radio Console:** a code search of `RTest/src` on 2026-09-25 found **no parser** of `browserRefreshOutcome` (docs and one kiosk test fixture only), so nothing should break. Per the guidance above, a consumer that treats an unrecognised value as a fault already handles it safely; one that switches on the old five values should add `SignedOut` → "needs a human sign-in". ⚠️ **MERGED, NOT DEPLOYED** until the owner deploys. |
| 2026-09-25 | Radio Console session (housekeeping) | **Two measured facts from the owner's 2026-09-25 cabinet sitting for Radio Console's `AUD-10` — one needs a RotaryPhone decision, one is a Radio Console change announced before it is made. No hci0/hci1 ownership, profile or WirePlumber change is made by this row.** **(1) ⚠ DECISION NEEDED FROM ROTARYPHONE — `bt_manager` accepted an HFP RFCOMM `NewConnection` for the Pixel (`B0:D5:FB:D2:0D:68`) on `/org/bluez/hci0`, Radio Console's adapter under this document, twice.** Read from the `rotary-phone.service` journal on `radio` (re-read by this session with a bounded `journalctl --since/--until` query): `16:23:15.399463 … [bt_manager] NewConnection: device=/org/bluez/hci0/dev_B0_D5_FB_D2_0D_68, fd=9 …` and `16:47:42.297247 … [bt_manager] NewConnection: device=/org/bluez/hci0/dev_B0_D5_FB_D2_0D_68, fd=9 …`. BlueZ `Profile1` registrations are system-wide, not per adapter, so the handler is offered connections on both. `scripts/bt_manager.py:511-513` on `main` still reads *"Accept HFP on any adapter — the phone may connect via hci0 or hci1. Closing the fd for the "wrong" adapter causes the phone to drop ALL HFP connections"*. **This is the same mechanism as the 2026-09-07 row above** (`docs/prompts/2026-09-07-hfp-on-music-adapter-drops-the-phone.md`); these are two further occurrences, not a new finding. **Ask: decide whether `bt_manager` should refuse (or avoid being offered) non-`hci1` connections** — the 09-07 row's caveat stands: simply re-enabling the rejection is known to drop HFP on `hci1` too, so the mechanism is RotaryPhone's call. Not established by this row: that these two connections caused any audio fault. **(2) Radio Console is changing to IGNORE `Device1` objects outside its own adapter** (RTest `AUD-30` / `AUD-31`). Measured the same sitting, correlating the two services' journals by timestamp: RotaryPhone logged `BLOCKED: B0:D5:FB:D2:0D:68 is already paired on hci0 — refusing on /org/bluez/hci1` at **16:23:47.239** and **16:48:14.824** (the message at `scripts/bt_manager.py:787-788`); `radio-api` logged `Bluetooth device disconnected … reason="Unknown"` **318 ms** and **347 ms** later (16:23:47.539, 16:48:15.137) while BlueZ still reported the Pixel connected on `hci0`, and at 16:48:15 that phantom disconnect **tore down playing A2DP** (source `Playing → Stopped`). Radio Console's device enumeration and `InterfacesAdded`/`InterfacesRemoved` handlers accept a `Device1` under any adapter, so an `hci1` path event for the same MAC reads to it as its own device. The planned change (not yet built) scopes those to the `hci0` path prefix. **RotaryPhone's `BLOCKED` refusal is correct behaviour under rule 8 and is NOT being asked to change** — the defect is Radio Console's reading of it. **No action needed from RotaryPhone for (2).** Evidence and timeline: RTest `docs/queue/AUD-30.md` § *CONFIRMED 2026-09-25*. **(3) Acknowledgement, no request: two items Radio Console's punch list carried as "never filed" turn out to be already delivered by RotaryPhone, so they are NOT being filed.** Checked against `main` by reading source: `XR-5` (bell-failure contract) — `BellInviteFailed` and `BellRecovered` hub events (`Server/Services/SignalRNotifierService.cs:164-182`, first added in `127c032`, merged via #68 on 2026-07-29), `LastBellFailure` on `/api/phone/status` (`Server/Controllers/PhoneController.cs:78`, `:91`), `Ht801LastCheckedUtc` (`Core/SystemStatus.cs:80`) and `POST /api/phone/bell-failure/ack` (`PhoneController.cs:122`); `XR-6` (voicemail audio `404` during a GV auth blackout) — `GvVoicemailController.GetAudio` answers `502` on a failed list (`GVBridge/Api/GvVoicemailController.cs:90-91`, commit `c602ece`, merged via #76 on 2026-09-08). The remaining `XR-5` gap is on Radio Console's side — its `BellHealth.Failed` still has no producer. RTest's record is corrected in its own housekeeping PR. |
